                      ...
                      .then((answer) => {
                        ...
                        connection.query(
                          `INSERT INTO employee (first_name, last_name, role_id, manager_id)
                            VALUES ("${empFirstName}", "${empLastName}", ${selectedRoleId}, ${
                            managerMap[answer.empManager]
                          })`,
                          (err, res) => {
                            if (err) throw err;
    ...
    .then((answer) => {
      connection.query(
        `INSERT INTO role (title, salary, department_id)
            ...
            FROM department
            WHERE department.name = "${answer.departmentID}"`,
        (err, res) => {
          if (err) throw err;
          console.log(
            ...
            connection.query(`SELECT title FROM role`, (err, res) => {
              if (err) {
                ...
                inquirer
                  ...
                  .then((answer) => {
                    connection.query(
                      `UPDATE employee SET role_id =(SELECT id FROM role WHERE title = "${answer.roleSelection}") WHERE id = ${selectedEmpId}`
                    );
                    console.log(
                      selectedEmp + ` is now a ${answer.roleSelection}`
                    );
                    mainMenu();
                  });
              }
...
const deleteEmployee = () => {
  ...
  inquirer
    .prompt([
      {
        ...
        validate: function (answer) {
          if (answer === "") {
            ...
            console.log(
              "You must enter the ID of the employee you wish to delete."
            );
          } else {
            return true;
          }
    ...
    .then((answer) => {
      ...
      console.log(
        `Employee ${answer.empDelete} has been removed from the company.`
      );
      mainMenu();
    });
};
const deleteDept = () => {
  ...
  inquirer
    .prompt([
      {
        ...
        validate: function (answer) {
          if (answer === "") {
            ...
            console.log(
              "You must enter the ID of the department you wish to delete."
            );
          } else {
            return true;
          }
    ...
    .then((answer) => {
      connection.query(
        `DELETE FROM department WHERE id = ${answer.dptDelete} `
      );
      console.log(
        `Department ${answer.dptDelete} has been removed from the company.`
      );
      mainMenu();
    });
};
